...
""".strip() % (fixed_ips['ip_address'],
    ...
    def test_ip_allocation_for_ipv6_2_subnet_slaac_mode(self):
        ...
        port_mac = port['port']['mac_address']
        subnet_id_1 = v6_subnet_1['subnet']['id']
        subnet_id_2 = v6_subnet_2['subnet']['id']
        ...
        eui_addr_2 = str(ipv6_utils.get_ipv6_addr_by_EUI64(cidr_2,
                                                           port_mac))
        self.assertIn({'subnet_id': subnet_id_1, 'ip_address': eui_addr_1},
                      fixed_ips)
        self.assertIn({'subnet_id': subnet_id_2, 'ip_address': eui_addr_2},
                      fixed_ips)
...
class TestSubnetsV2(NeutronDbPluginV2TestCase):
    ...
    def _create_slaac_subnet(self, cidr='fe80::/64', gateway='fe80::1'):
        # Create an IPv6 SLAAC subnet
        res = self._create_network(fmt=self.fmt, name='net',
                                   admin_state_up=True)
        network = self.deserialize(self.fmt, res)
        subnet = self._make_subnet(self.fmt, network, gateway=gateway,
                                   cidr=cidr, ip_version=6,
                                   ipv6_ra_mode=constants.IPV6_SLAAC,
                                   ipv6_address_mode=constants.IPV6_SLAAC)
        return subnet

    def _create_port_on_slaac_subnet(self, net_id, port_owner=None,
                                     fixed_ips=None):
        kwargs = dict()
        if port_owner: kwargs["device_owner"] = port_owner
        if fixed_ips: kwargs["fixed_ips"] = fixed_ips
        res = self._create_port(self.fmt, net_id=net_id, **kwargs)
        port = self.deserialize(self.fmt, res)
        self.assertEqual(1, len(port['port']['fixed_ips']))
    ...
    def test_delete_subnet_ipv6_slaac_port_exists(self):
        ...
        subnet = self._create_slaac_subnet()
        port = self._create_port_on_slaac_subnet(
                net_id=subnet['subnet']['network_id'])
        # Delete the subnet
        req = self.new_delete_request('subnets', subnet['subnet']['id'])
        res = req.get_response(self.api)
    ...
    def test_delete_subnet_ipv6_slaac_router_port_exists(self):
        ...
        subnet = self._create_slaac_subnet(cidr='dead:beef::/64',
                                           gateway='dead:beef::1')
        port = self._create_port_on_slaac_subnet(
                net_id=subnet['subnet']['network_id'],
                port_owner=constants.DEVICE_OWNER_ROUTER_INTF,
                fixed_ips=[{
                    'subnet_id': subnet['subnet']['id'],
                ...
                with self.port(subnet=v4_subnet,
                    ...
                    with self.subnet(network=network, cidr='fe80::/64',
                        ...
                        if device_owner in constants.ROUTER_INTERFACE_OWNERS:
                            # Router interfaces should not have been updated
                            # with addresses from the new auto-address subnet
                            self.assertEqual(1,
...
class TestNeutronDbPluginV2(base.BaseTestCase):
    ...
    def _test__allocate_ips_for_port(self, subnets, port, expected):
        ...
        with mock.patch.object(db_base_plugin_v2.NeutronDbPluginV2,
                               '_get_subnets_by_network') as get_subnets:
            with mock.patch.object(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def test_allocate_ips_for_port_2_slaac_subnets(self):
        subnets = [
            {
                'cidr': u'2001:100::/64',
                'enable_dhcp': True,
