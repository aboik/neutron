...
class L3NatTestCaseBase(L3NatTestCaseMixin):
    ...
    def test_router_update_gateway_add_multiple_prefixes_ipv6(self):
        with self.network() as n:
            with self.subnet(network=n) as s1, self.subnet(network=n,
                    ip_version=6, cidr='2001:db8::/32') as s2, (
                            self.router()) as r:
                self._set_net_external(n['network']['id'])
                res1 = self._add_external_gateway_to_router(
                        r['router']['id'],
                        n['network']['id'],
                        ext_ips=[{'subnet_id': s1['subnet']['id']}])
                fip1_1 = (res1['router']['external_gateway_info']
                          ['external_fixed_ips'][0])
                self.assertEqual(s1['subnet']['id'], fip1_1['subnet_id'])
                res2 = self._add_external_gateway_to_router(
                        r['router']['id'],
                        n['network']['id'],
                        ext_ips=[{'ip_address': fip1_1['ip_address'],
                                  'subnet_id': s1['subnet']['id']},
                                 {'subnet_id': s2['subnet']['id']}])
                fip1_2 = (res2['router']['external_gateway_info']
                          ['external_fixed_ips'][0])
                fip2 = (res2['router']['external_gateway_info']
                        ['external_fixed_ips'][1])
                self.assertEqual(fip1_1, fip1_2)
                self.assertEqual(s2['subnet']['id'], fip2['subnet_id'])
                self.assertNotEqual(fip1_2['subnet_id'],
                                    fip2['subnet_id'])
                self.assertNotEqual(fip1_2['ip_address'],
                                    fip2['ip_address'])
    ...
    def test_router_update_gateway_upon_subnet_create_ipv6(self):
        with self.network() as n:
            with self.subnet(network=n) as s1, self.router() as r:
                ...
                self.assertEqual(fip1_1, fip1_2)
                self.assertEqual(s2['subnet']['id'], fip2['subnet_id'])
                self.assertNotEqual(fip1_2['subnet_id'], fip2['subnet_id'])
                self.assertNotEqual(fip1_2['ip_address'], fip2['ip_address'])
